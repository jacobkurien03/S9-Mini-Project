...
const sendMail = async (id, email, option) => {
  ...
  if (option === "email verification") {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: "Confirm your email for Amazing Shopping Cart", // Subject line
      html: `<div>
  ...
  // send a mail for resetting password if forgot password
    ...
    const mailOptions = {
      from: process.env.EMAIL, // sender address
      to: email,
      subject: "Reset Password for Amazing", // Subject line
      html: `<div>
					<h2>Reset Password for your Amazing account</h2>
					<br/>
