...
const ProductListPage = ({ history, match }) => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you wanna delete this Address?")) {
      ...
      let response = await axios.post(`/api/users/deleteAddress`, data);
  ...
  async function getAddress() {
    ...
    let response = await axios.post("/api/users/showAddress", data);
    if (response.status === 200) {
                ...
                address.map((addres) => {
                  return (
                    <tr key={addres.address}>
                      ...
                      >
                        <LinkContainer to={`/updateAddress/${addres._id}`}>
